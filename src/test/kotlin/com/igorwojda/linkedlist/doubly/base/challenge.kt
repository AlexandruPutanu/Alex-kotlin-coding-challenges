...
private class DoublyLinkedList<E> : Iterable<Node<E>> {
    ...
    fun getAt(index: Int): Node<E>? {
        ...
        var node = head
        var indexSoFar = 0
        while (node != null && indexSoFar < index) {
            node = node.next
            indexSoFar++
        }
        return node
    ...
    fun setAt(data: E, index: Int) {
        ...
        if (head == null || index == 0) {
            head = Node(data, next = head?.next)
            return
        }
        var node = head
        var indexSoFar = 0
        while (node?.next != null && indexSoFar < index - 1) {
            node = node.next
            indexSoFar++
        }
        if (indexSoFar == index - 1) {
            val replacedNode = node?.next
            val updatedNode = Node(data, prev = node, next = replacedNode?.next)
    ...
    fun removeAt(index: Int) {
        ...
        if (indexSoFar == index - 1) {
            val afterRemovedNode = node?.next?.next
            afterRemovedNode?.prev = node
    ...
    fun insertAt(data: E, index: Int) {
        ...
        if (indexSoFar == index - 1) {
            val replacedNode = node?.next
            val updatedNode = Node(data, prev = node)
            replacedNode?.prev = updatedNode
            updatedNode.next = replacedNode
            node?.next = updatedNode
        }
    }

    override fun iterator(): Iterator<Node<E>> = object : Iterator<Node<E>> {
        var node = head
    ...
    operator fun plus(other: DoublyLinkedList<E>): DoublyLinkedList<E> {
        val dataList = this.map { it.data }.toList() + other.map { it.data }.toList()
        val linkedList = DoublyLinkedList<E>()
        dataList.forEach { linkedList.insertLast(it) }
        return linkedList
    }
}
